...
public class Import {
	...
	public Import (LayoutManager layout, JFrame frame, GridBagConstraints c, String path, boolean valChange, long fileSizeLimit) {
		this.layout=layout;
		this.frame=frame;
		this.c=c;
		this.path=path;
...
//	 String tabHeaderContent[][]=null;//original placement
   ...
   //   File fl = new File("Global-Besoin-Materiel-LTE-ooredoo-18072019-update-RFI-Power-Solution - Copy.xlsx");
     ...
     if((fileLength<=fileSizeLimit)|fileSizeLimit==0) {
      if (fileSizeLimit==0)
     	 JOptionPane.showMessageDialog(null, "File size limit undefined, this constraint is disabled");    	  
...
//            	  System.out.println("lastcellfirstrow "+ row.getLastCellNum()+" hasnext "+cellIt.hasNext());
      ...
      //                System.out.println("redrow "+ rowMax+" actualrow "+cell.getRowIndex()+" redcolumn "+ columnMax+" actualcolumn "+cell.getColumnIndex());
             ...
             //        System.out.println("colour check "+((XSSFColor)color).getARGBHex());//should be enable only for colour identification
             //         System.out.println("colour check2 "+((XSSFColor)color).getARGBHex().equals("FFFFC000"));//should be enable only for colour identification   
                      
                      
                         if (color==null);
                         
                         else if (((XSSFColor)color).getARGBHex().equals("FFFF0000")) {//red1
...
//                	 System.out.println("Colour is null");
                 else if (((XSSFColor)color).getARGBHex().equals("FFFF0000")) {//red2
//            	  System.out.println("passredrow "+ rowMax+" actualrow "+cell.getRowIndex()+" redcolumn "+ columnMax+" actualcolumn "+cell.getColumnIndex() );
...
//          			System.out.println(tabHeader[k*3+2][m]+" at "+ tabHeader[k*3][m]+" "+ tabHeader[k*3+1][m] +" belongs to "+tabHeader[k*3-1][t]+" at "+tabHeader[k*3-3][t]+" "+tabHeader[k*3-2][t]);
  ...
  //  					System.out.println("t content is empty2");
         ...
         JOptionPane.showMessageDialog(null, fileLength+" is higher than the defined threshhold, please try another file "+fileSizeLimit);
            }
